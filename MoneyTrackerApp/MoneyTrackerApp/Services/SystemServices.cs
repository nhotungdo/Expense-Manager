...
/// <summary>
/// Service for AI-powered financial suggestions
/// </summary>
...
public class AiAdvisorService : IAiAdvisorService
{
    ...
    public async Task GenerateSuggestionsAsync(long userId, GenerateAiSuggestionsDto? dto = null)
    {
        ...
        var startDate = dto?.StartDate ?? endDate.AddMonths(-3);
        ...
        if (totalIncome > 0)
        {
            ...
            if (savingsRate < 20)
            {
                suggestions.Add(new AiSuggestion
                {
                    ...
                    Suggestion = $"Your savings rate is {savingsRate:N1}%. Consider saving at least 20% of your income for financial security.",
                    CreatedAt = DateTime.UtcNow
                });
            }
        }
        ...
        if (topCategory != null)
        {
            ...
            var percentage = totalExpense > 0 ? (categoryAmount / totalExpense) * 100 : 0;
            ...
            if (percentage > 30)
            {
                suggestions.Add(new AiSuggestion
                {
                    ...
                    Suggestion = $"You're spending {percentage:N1}% of your budget on {topCategory.Key}. Consider reviewing this category for potential savings.",
                    CreatedAt = DateTime.UtcNow
                });
            }
        }

        // Suggestion 3: Budget recommendation
        var hasNoBudgets = !await _context.Budgets.AnyAsync(b => b.UserId == userId);
        if (hasNoBudgets && totalExpense > 0)
        {
            suggestions.Add(new AiSuggestion
            {
                UserId = userId,
                SuggestionType = "Budget",
                Suggestion = "You don't have any budgets set up. Creating budgets can help you track and control your spending better.",
                CreatedAt = DateTime.UtcNow
            });
        }

        // Add suggestions to database
        _context.AiSuggestions.AddRange(suggestions);
        await _context.SaveChangesAsync();
    }
}
