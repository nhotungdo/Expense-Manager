...
CREATE TABLE [Accounts] (
    ...
    [IncludeInTotal] bit NOT NULL DEFAULT 1,
    [CreatedAt] datetime2 NULL DEFAULT GETUTCDATE(),
    [UpdatedAt] datetime2 NULL,
...
CREATE TABLE [Transactions] (
    ...
    [PairedAccountId] bigint NULL,
    [PairedTransactionId] bigint NULL,
...
-- =============================================
-- 8. SCHEDULED TRANSACTIONS (RECURRING) TABLE
-- =============================================
...
CREATE TABLE [Debts] (
    [Id] bigint IDENTITY(1,1) NOT NULL,
    [UserId] bigint NOT NULL,
    [DebtType] int NOT NULL, -- 1 = I owe, 2 = I am owed
    [Name] nvarchar(100) NOT NULL,
...
CREATE TABLE [SharedAccounts] (
    ...
    CONSTRAINT [UK_SharedAccounts_AccountId_UserId] UNIQUE ([AccountId], [UserId])
);
GO
...
CREATE TABLE [Investments] (
    ...
    CONSTRAINT [FK_Investments_Accounts_AccountId] FOREIGN KEY ([AccountId]) REFERENCES [Accounts] ([Id]) ON DELETE NO ACTION
);
GO
...
CREATE TABLE [BankConnections] (
    ...
    CONSTRAINT [FK_BankConnections_Accounts_AccountId] FOREIGN KEY ([AccountId]) REFERENCES [Accounts] ([Id]) ON DELETE NO ACTION
);
GO
...
-- =============================================
-- 27. GROUP TRANSACTION SPLITS TABLE
-- =============================================
...
-- =============================================
-- 28. SERVICE PACKAGES & SUBSCRIPTIONS TABLES
-- =============================================
...
CREATE NONCLUSTERED INDEX [IX_ServicePackages_PackageType] ON [ServicePackages]([PackageType] ASC);
CREATE NONCLUSTERED INDEX [IX_ServicePackages_IsActive] ON [ServicePackages]([IsActive] ASC);
GO
...
CREATE TABLE [Subscriptions](
    ...
    CONSTRAINT [FK_Subscriptions_ServicePackages] FOREIGN KEY([PackageId]) REFERENCES [ServicePackages]([Id])
);

CREATE NONCLUSTERED INDEX [IX_Subscriptions_UserId] ON [Subscriptions]([UserId] ASC);
CREATE NONCLUSTERED INDEX [IX_Subscriptions_PackageId] ON [Subscriptions]([PackageId] ASC);
CREATE NONCLUSTERED INDEX [IX_Subscriptions_Status] ON [Subscriptions]([Status] ASC);
CREATE NONCLUSTERED INDEX [IX_Subscriptions_EndDate] ON [Subscriptions]([EndDate] ASC);
GO
...
CREATE NONCLUSTERED INDEX [IX_Payments_SubscriptionId] ON [Payments]([SubscriptionId] ASC);
CREATE NONCLUSTERED INDEX [IX_Payments_Status] ON [Payments]([Status] ASC);
CREATE NONCLUSTERED INDEX [IX_Payments_TransactionId] ON [Payments]([TransactionId] ASC);
GO

-- =============================================
-- 29. PAYMENT TRANSACTIONS TABLE
-- =============================================
CREATE TABLE [PaymentTransactions](
    ...
    CONSTRAINT [FK_PaymentTransactions_Users] FOREIGN KEY([UserId]) REFERENCES [Users]([Id]) ON DELETE CASCADE,
    CONSTRAINT [FK_PaymentTransactions_ServicePackages] FOREIGN KEY([PackageId]) REFERENCES [ServicePackages]([Id])
);

CREATE NONCLUSTERED INDEX [IX_PaymentTransactions_UserId] ON [PaymentTransactions]([UserId] ASC);
CREATE NONCLUSTERED INDEX [IX_PaymentTransactions_PackageId] ON [PaymentTransactions]([PackageId] ASC);
CREATE NONCLUSTERED INDEX [IX_PaymentTransactions_Status] ON [PaymentTransactions]([Status] ASC);
CREATE NONCLUSTERED INDEX [IX_PaymentTransactions_GatewayTransactionId] ON [PaymentTransactions]([GatewayTransactionId] ASC);
CREATE NONCLUSTERED INDEX [IX_PaymentTransactions_RequestTimestamp] ON [PaymentTransactions]([RequestTimestamp] ASC);
GO
...
IF OBJECT_ID('tr_Budgets_UpdatedAt', 'TR') IS NOT NULL DROP TRIGGER [tr_Budgets_UpdatedAt];
IF OBJECT_ID('tr_SystemSettings_UpdatedAt', 'TR') IS NOT NULL DROP TRIGGER [tr_SystemSettings_UpdatedAt];
IF OBJECT_ID('tr_Accounts_UpdatedAt', 'TR') IS NOT NULL DROP TRIGGER [tr_Accounts_UpdatedAt];
IF OBJECT_ID('tr_SavingsGoals_UpdatedAt', 'TR') IS NOT NULL DROP TRIGGER [tr_SavingsGoals_UpdatedAt];
IF OBJECT_ID('tr_ScheduledTransactions_UpdatedAt', 'TR') IS NOT NULL DROP TRIGGER [tr_ScheduledTransactions_UpdatedAt];
IF OBJECT_ID('tr_Debts_UpdatedAt', 'TR') IS NOT NULL DROP TRIGGER [tr_Debts_UpdatedAt];
IF OBJECT_ID('tr_Investments_UpdatedAt', 'TR') IS NOT NULL DROP TRIGGER [tr_Investments_UpdatedAt];
IF OBJECT_ID('tr_Transactions_UpdateAccountBalance', 'TR') IS NOT NULL DROP TRIGGER [tr_Transactions_UpdateAccountBalance];
IF OBJECT_ID('tr_SavingsTransactions_UpdateGoal', 'TR') IS NOT NULL DROP TRIGGER [tr_SavingsTransactions_UpdateGoal];
IF OBJECT_ID('tr_ServicePackages_UpdatedAt', 'TR') IS NOT NULL DROP TRIGGER [tr_ServicePackages_UpdatedAt];
...
CREATE TRIGGER [tr_Users_UpdatedAt] ON [Users] AFTER UPDATE AS BEGIN SET NOCOUNT ON; UPDATE [Users] SET [UpdatedAt] = GETUTCDATE() FROM [Users] u INNER JOIN inserted i ON u.[Id] = i.[Id]; END; GO
CREATE TRIGGER [tr_Categories_UpdatedAt] ON [Categories] AFTER UPDATE AS BEGIN SET NOCOUNT ON; UPDATE [Categories] SET [UpdatedAt] = GETUTCDATE() FROM [Categories] c INNER JOIN inserted i ON c.[Id] = i.[Id]; END; GO
CREATE TRIGGER [tr_Transactions_UpdatedAt] ON [Transactions] AFTER UPDATE AS BEGIN SET NOCOUNT ON; UPDATE [Transactions] SET [UpdatedAt] = GETUTCDATE() FROM [Transactions] t INNER JOIN inserted i ON t.[Id] = i.[Id]; END; GO
CREATE TRIGGER [tr_Budgets_UpdatedAt] ON [Budgets] AFTER UPDATE AS BEGIN SET NOCOUNT ON; UPDATE [Budgets] SET [UpdatedAt] = GETUTCDATE() FROM [Budgets] b INNER JOIN inserted i ON b.[Id] = i.[Id]; END; GO
CREATE TRIGGER [tr_SystemSettings_UpdatedAt] ON [SystemSettings] AFTER UPDATE AS BEGIN SET NOCOUNT ON; UPDATE [SystemSettings] SET [UpdatedAt] = GETUTCDATE() FROM [SystemSettings] s INNER JOIN inserted i ON s.[Id] = i.[Id]; END; GO
CREATE TRIGGER [tr_Accounts_UpdatedAt] ON [Accounts] AFTER UPDATE AS BEGIN SET NOCOUNT ON; UPDATE [Accounts] SET [UpdatedAt] = GETUTCDATE() FROM [Accounts] a INNER JOIN inserted i ON a.[Id] = i.[Id]; END; GO
CREATE TRIGGER [tr_SavingsGoals_UpdatedAt] ON [SavingsGoals] AFTER UPDATE AS BEGIN SET NOCOUNT ON; UPDATE [SavingsGoals] SET [UpdatedAt] = GETUTCDATE() FROM [SavingsGoals] s INNER JOIN inserted i ON s.[Id] = i.[Id]; END; GO
CREATE TRIGGER [tr_ScheduledTransactions_UpdatedAt] ON [ScheduledTransactions] AFTER UPDATE AS BEGIN SET NOCOUNT ON; UPDATE [ScheduledTransactions] SET [UpdatedAt] = GETUTCDATE() FROM [ScheduledTransactions] s INNER JOIN inserted i ON s.[Id] = i.[Id]; END; GO
CREATE TRIGGER [tr_Debts_UpdatedAt] ON [Debts] AFTER UPDATE AS BEGIN SET NOCOUNT ON; UPDATE [Debts] SET [UpdatedAt] = GETUTCDATE() FROM [Debts] d INNER JOIN inserted i ON d.[Id] = i.[Id]; END; GO
CREATE TRIGGER [tr_Investments_UpdatedAt] ON [Investments] AFTER UPDATE AS BEGIN SET NOCOUNT ON; UPDATE [Investments] SET [UpdatedAt] = GETUTCDATE() FROM [Investments] inv INNER JOIN inserted i ON inv.[Id] = i.[Id]; END; GO
CREATE TRIGGER [tr_ServicePackages_UpdatedAt] ON [ServicePackages] AFTER UPDATE AS BEGIN SET NOCOUNT ON; UPDATE [ServicePackages] SET [UpdatedAt] = GETUTCDATE() FROM [ServicePackages] sp INNER JOIN inserted i ON sp.[Id] = i.[Id]; END; GO
...
BEGIN
    ...
    + ISNULL((SELECT SUM(CASE WHEN t.[TransactionType] = 1 THEN t.[Amount] WHEN t.[TransactionType] = 2 THEN -t.[Amount] WHEN t.[TransactionType] = 3 THEN -t.[Amount] ELSE 0 END) FROM [Transactions] t WHERE t.[AccountId] = acc.[Id]), 0) 
    + ISNULL((SELECT SUM(CASE WHEN t.[TransactionType] = 3 THEN t.[Amount] ELSE 0 END) FROM [Transactions] t WHERE t.[PairedAccountId] = acc.[Id]), 0)
    FROM [Accounts] acc
...
BEGIN
    ...
    UPDATE sg
    SET [CurrentAmount] = ISNULL((SELECT SUM(Amount) FROM [SavingsTransactions] st WHERE st.SavingsGoalId = sg.Id), 0)
...
-- INSERT DEFAULT DATA
-- =============================================
-- Default Roles and Admin
INSERT INTO [AspNetRoles] ([Id], [Name], [NormalizedName], [ConcurrencyStamp]) VALUES ('1', 'Admin', 'ADMIN', NEWID()), ('2', 'User', 'USER', NEWID());
GO

INSERT INTO [Users] ([GoogleId], [UserName], [Email], [FullName], [Role], [Enabled], [CreatedAt]) VALUES ('admin', 'admin', 'nhotungdo89@gmail.com', 'System Administrator', 'Admin', 1, GETUTCDATE());
GO

INSERT INTO [AspNetUserRoles] ([UserId], [RoleId]) SELECT u.[Id], r.[Id] FROM [Users] u, [AspNetRoles] r WHERE u.[GoogleId] = 'admin' AND r.[Name] = 'Admin';
GO
...
BEGIN
    ...
    VALUES 
    (1, N'Gói Miễn Phí', N'Hoàn hảo để bắt đầu quản lý tài chính cá nhân', 0, NULL, 365, 1, N'["Theo dõi thu chi cơ bản","Tối đa 3 ví","Báo cáo hàng tháng","Hỗ trợ qua email","Lưu trữ 100 giao dịch"]', 0, 1, NULL, NULL, 1, GETUTCDATE(), 0, 100, 3, 3, 0, 0, 0, 0),
    (2, N'Gói Cơ Bản', N'Dành cho người dùng cá nhân muốn quản lý tốt hơn', 99000, 149000, 30, 1, N'["Tất cả tính năng Miễn Phí","Không giới hạn ví","Báo cáo chi tiết","Phân loại tự động","Lưu trữ không giới hạn","Xuất báo cáo Excel/PDF","Hỗ trợ ưu tiên"]', 0, 1, N'Giảm 33%', N'discount', 2, GETUTCDATE(), 1, -1, -1, -1, 1, 0, 0, 0),
    (3, N'Gói Chuyên Nghiệp', N'Giải pháp toàn diện cho quản lý tài chính chuyên nghiệp', 199000, 299000, 30, 1, N'["Tất cả tính năng Cơ Bản","AI phân tích chi tiêu","Dự báo tài chính","Quản lý đầu tư","Theo dõi nợ & tiết kiệm","Chia sẻ chi tiêu nhóm","Tích hợp ngân hàng","Hỗ trợ 24/7","Tư vấn tài chính cá nhân"]', 1, 1, N'Phổ biến nhất', N'popular', 3, GETUTCDATE(), 2, -1, -1, -1, 1, 1, 1, 1),
    (4, N'Gói Doanh Nghiệp', N'Giải pháp cho doanh nghiệp và nhóm làm việc', 499000, NULL, 30, 1, N'["Tất cả tính năng Chuyên Nghiệp","Quản lý nhiều người dùng","Phân quyền chi tiết","API tích hợp","Báo cáo tùy chỉnh","Sao lưu tự động","Bảo mật nâng cao","Đào tạo & onboarding","Account manager riêng"]', 0, 1, NULL, NULL, 4, GETUTCDATE(), 3, -1, -1, -1, 1, 1, 1, 1),
    (5, N'Gói Năm - Cơ Bản', N'Tiết kiệm 20% khi đăng ký theo năm', 950000, 1188000, 365, 12, N'["Tất cả tính năng Gói Cơ Bản","Thanh toán 1 lần/năm","Tiết kiệm 238.000đ","Ưu tiên cập nhật tính năng mới"]', 0, 1, N'Tiết kiệm 20%', N'discount', 5, GETUTCDATE(), 1, -1, -1, -1, 1, 0, 0, 0),
    (6, N'Gói Năm - Chuyên Nghiệp', N'Tiết kiệm 25% khi đăng ký theo năm', 1790000, 2388000, 365, 12, N'["Tất cả tính năng Gói Chuyên Nghiệp","Thanh toán 1 lần/năm","Tiết kiệm 598.000đ","Tặng 1 tháng sử dụng","Ưu tiên hỗ trợ VIP"]', 1, 1, N'Ưu đãi nhất', N'popular', 6, GETUTCDATE(), 2, -1, -1, -1, 1, 1, 1, 1);
